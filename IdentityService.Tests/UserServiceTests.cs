...
public class UserServiceTests
{
    ...
    {
        // Arrange
        var loginModel = new LoginRequestModel { Email = "test@test.se", Password = "illegals" };
        ...
        _mockSignInManager
            .Setup(x => x.PasswordSignInAsync(user.UserName, loginModel.Password, false, false))
            .ReturnsAsync(SignInResult.Failed);

        _mockUserManager.Setup(m => m.AccessFailedAsync(user)).ReturnsAsync(IdentityResult.Success);
        _mockUserManager
            .SetupSequence(m => m.GetAccessFailedCountAsync(user))
            .ReturnsAsync(1)
            .ReturnsAsync(2)
            .ReturnsAsync(3);
        _mockUserManager
            ...
            .ReturnsAsync(true);
        _mockUserManager
            .Setup(m => m.GetLockoutEndDateAsync(user))
    ...
    public async Task Login_Fails_When_User_Does_Not_Exist_In_Database()
    {
        ...
        _mockUserManager
            .Setup(x => x.FindByEmailAsync(loginModel.Email))
    ...
    public async Task Login_With_RememberMe_Keeps_User_Signed_In()
    {
        ...
        _mockSignInManager
            .Setup(x =>
                x.PasswordSignInAsync(user.UserName, loginModel.Password, rememberMe, false)
            )
            .ReturnsAsync(SignInResult.Success);
        ...
        _mockSignInManager.Verify(
            x => x.PasswordSignInAsync(user.UserName, loginModel.Password, rememberMe, false),
            Times.Once
        );
    }
}
